...
import (
	...
	callsUtil "github.com/OPN-Network/chainsafe-core/chains/evm/calls"
	...
	"github.com/OPN-Network/chainsafe-core/chains/evm/cli/initialize"
...
func WithdrawCmd(cmd *cobra.Command, args []string, contract *bridge.BridgeContract) error {
	...
	log.Info().Msgf("%s tokens were withdrawn from handler contract %s into recipient %s; tx hash: %s", RealAmount, Handler, Recipient, h.Hex())
	return nil
}
